...
export const NUEVO_PIC_PRODUCTIVO_EVENT = "pic:nuevo-pedido-productivo";

type PedidoProductivoRow = {
  id?: string | number;
  sector?: string | null;
...
export default function NuevoPicProductivoAlertListener() {
  useEffect(() => {
    ...
    const avisar = (pedido: PedidoProductivoRow, mensaje: string) => {
      window.dispatchEvent(
        new CustomEvent(NUEVO_PIC_PRODUCTIVO_EVENT, { detail: pedido })
      );
      setTimeout(() => {
    ...
    const setup = async () => {
      ...
      channel = supabase
        .channel(`nuevo-pic-productivo-alert:${user.id}`)
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "pedidos_productivos" },
          (payload) => {
            const pedido = (payload.new ?? {}) as PedidoProductivoRow;
            const anterior = (payload.old ?? {}) as PedidoProductivoRow;
